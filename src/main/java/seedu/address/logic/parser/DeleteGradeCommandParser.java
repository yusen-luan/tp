...
public class DeleteGradeCommandParser implements Parser<DeleteGradeCommand> {
    ...
    public DeleteGradeCommand parse(String args) throws ParseException {
        ...
        List<String> assignmentNames = argMultimap.getAllValues(PREFIX_GRADE);
        ...
        // Validate that assignment names are not blank
        ...
        Set<String> assignmentNamesToDelete = new HashSet<>();
        for (String assignmentName : assignmentNames) {
            assignmentNamesToDelete.add(assignmentName.trim());
        }

        return new DeleteGradeCommand(index, assignmentNamesToDelete);
    }
